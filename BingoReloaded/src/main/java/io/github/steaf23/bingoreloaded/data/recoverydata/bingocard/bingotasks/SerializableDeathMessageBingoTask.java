...
import io.github.steaf23.bingoreloaded.tasks.bingotasks.DeathMessageBingoTask;
...
@SerializableAs("DeathMessageBingoTask")
public record SerializableDeathMessageBingoTask(
        UUID completedBy,
        int completedAt,
        boolean voided,
        DeathMessageTask taskData
    ...
    public SerializableDeathMessageBingoTask(DeathMessageBingoTask task) {
        this(
    ...
    public static SerializableDeathMessageBingoTask deserialize(Map<String, Object> data)
    {
        ...
        return new SerializableDeathMessageBingoTask(
                completedBy,
                (Integer) data.getOrDefault(COMPLETED_AT_ID, -1),
                (Boolean) data.getOrDefault(VOIDED_ID, false),
                (DeathMessageTask) data.getOrDefault(DATA_ID, null)
        );
    }
