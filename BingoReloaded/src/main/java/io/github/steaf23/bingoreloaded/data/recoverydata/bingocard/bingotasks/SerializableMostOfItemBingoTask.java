...
@SerializableAs("MostOfItemBingoTask")
public record SerializableMostOfItemBingoTask(
        UUID completedBy,
        int completedAt,
        boolean voided,
        MostOfItemTask taskData,
    ...
    public SerializableMostOfItemBingoTask(MostOfItemBingoTask task) {
        this(
    ...
    public static SerializableMostOfItemBingoTask deserialize(Map<String, Object> data)
    {
        ...
        return new SerializableMostOfItemBingoTask(
                completedBy,
                (Integer) data.getOrDefault(COMPLETED_AT_ID, -1),
                (Boolean) data.getOrDefault(VOIDED_ID, false),
                (MostOfItemTask) data.getOrDefault(DATA_ID, null),
    ...
    public BingoTask<?> toBingoTask(BingoSession session, ChildHavingBingoTask<?> parentTask) {
        MostOfItemBingoTask task = new MostOfItemBingoTask(taskData, parentTask, teamCount);
        if (completedBy != null) {
