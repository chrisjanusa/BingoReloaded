...
@SerializableAs("StatisticBingoTask")
public record SerializableStatisticBingoTask(
        UUID completedBy,
        int completedAt,
        boolean voided,
        StatisticTask taskData
    ...
    public SerializableStatisticBingoTask(StatisticBingoTask task) {
        this(
    ...
    public static SerializableStatisticBingoTask deserialize(Map<String, Object> data)
    {
        ...
        return new SerializableStatisticBingoTask(
                completedBy,
                (Integer) data.getOrDefault(COMPLETED_AT_ID, -1),
                (Boolean) data.getOrDefault(VOIDED_ID, false),
                (StatisticTask) data.getOrDefault(DATA_ID, null)
        );
    }
