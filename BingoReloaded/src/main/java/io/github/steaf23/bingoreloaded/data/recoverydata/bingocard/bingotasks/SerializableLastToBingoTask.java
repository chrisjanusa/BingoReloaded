...
@SerializableAs("LastToBingoTask")
public record SerializableLastToBingoTask(
        UUID completedBy,
        int completedAt,
        boolean voided,
        LastToTask taskData,
    ...
    public SerializableLastToBingoTask(LastToBingoTask task) {
        this(
    ...
    public static SerializableLastToBingoTask deserialize(Map<String, Object> data)
    {
    ...
    public BingoTask<?> toBingoTask(BingoSession session, ChildHavingBingoTask<?> parentTask) {
        LastToBingoTask task = new LastToBingoTask(taskData, parentTask, session.teamManager.getActiveTeams());
        if (completedBy != null) {
